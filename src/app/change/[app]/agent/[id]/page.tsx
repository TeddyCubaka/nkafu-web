...
const ListModelPage = () => {
  ...
  useEffect(() => {
    ...
    const requester = async () => {
      ...
      const response: { code: number; message: string; data: any; meta: any } =
        await httpClient.get(`/change/${params.app}/agent`);
  ...
  const handleConfirm = async () => {
    ...
    const response: any | false = await httpClient.delete(
      `/delete/${params.app}/agent/${params.id}`
    );
  ...
  return (
    <div className="p-10 flex flex-col gap-5">
      <div>
        <div></div>
        <Form
          title={`Mise à jour de l'agent : ${data.firstName} ${data.lastName}`}
          inputs={inputs}
          ...
          actions={
            <div>
              <Button
                ...
                variant="primary"
                onClick={() => router.push(`/list/${params.app}/agent`)}
              >
                Annuler
              </Button>
              <Button
